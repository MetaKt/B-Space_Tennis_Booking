...
### Pending / In Progress
- [ ] **ThaiBulkSMS approved sender name** — still sends as `"Demo"`. Getting a real branded sender (e.g. `BSPACE`, ≤11 characters) requires submitting a request through ThaiBulkSMS's dashboard/support and waiting for approval (days, not minutes) — not a code change, just an env var swap once approved. Not confirmed whether the earlier "generic test message substitution" problem (§ThaiBulkSMS section above, `force=standard` era) still happens under `force=corporate` — worth a real-content check next time an OTP is sent to a live phone.
