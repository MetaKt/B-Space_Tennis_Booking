...
**Version:** 1.5
**Date:** 2026-07-11
**Status:** Deployed — live at `https://b-space-tennis-booking.vercel.app` (frontend) and `https://b-spacetennisbooking-production.up.railway.app` (backend). See §15.1 for remaining pre-launch items (final full smoke test, ThaiBulkSMS approved sender name).
...
| `/api/settings` | routes/settings.js | get all (admin only), get public (any logged-in user, not admin-gated — still requires a valid token), update, bulk-upsert, delete |
...
- [x] **Backend deployed to Railway** — live at `https://b-spacetennisbooking-production.up.railway.app`, Root Directory `backend`, region Singapore. Verified with real HTTP requests: `/api/health` returns `200`, protected routes correctly `401` without a token, `/api/auth/login` correctly `404`s for the (intentionally unseeded) production DB.
- [x] **Rotate JWT secrets** — fresh 48-byte random `JWT_SECRET`/`JWT_REFRESH_SECRET` generated and set directly in Railway's env vars; local `.env` dev values were never reused.
- [x] **Frontend deployed to Vercel** — live at `https://b-space-tennis-booking.vercel.app`, Root Directory `frontend`. Verified with a real end-to-end login: real SMS received, reached the Verify OTP screen.
- [x] **ThaiBulkSMS `force=corporate` fix** — sends started failing in production with `ERROR_SENDER` ("Sender is invalid") even though the exact same request succeeded when tested manually earlier. Root cause: the account carries both leftover trial ("standard") and real ("corporate") credit, and once that's the case, sending with `force: standard` (the original hardcoded value in `utils/otp.js`) gets rejected — `force: corporate` is required. Verified directly against the ThaiBulkSMS API before touching the deployed code.
- [x] **Socket.IO CORS restricted** — `lib/socket.js` now mirrors the REST API's origin allowlist (dev: localhost + private-LAN; prod: `FRONTEND_URL` only) instead of `origin: '*'`. Verified both locally and on production: allowed origins get a successful handshake with the correct `Access-Control-Allow-Origin` header, disallowed origins get `400`.
- [x] **Removed leaked dev-only UI text** — `OTPPage.js` unconditionally rendered "DEV MODE: Check server console for OTP" regardless of environment, so real production users saw a message referencing a console they can't access. Gated behind `process.env.NODE_ENV === 'development'`; confirmed the string is fully absent (dead-code-eliminated, not just hidden) from the production bundle.

**Deployment debugging notes** (real issues hit and fixed while getting Railway + Vercel talking to each other — worth knowing if this ever needs to be re-done):
- Railway's public domain can silently detach from a service (`Public Networking` showed no domain at all) — symptom was every request, even `/api/health`, returning `{"status":"error","code":404,"message":"Application not found"}` with `x-railway-fallback: true` from `railway-hikari`, despite the service itself showing "Online." Fix: click "Generate Domain" again under Settings → Networking.
- `FRONTEND_URL` on Railway had a trailing slash (`.../vercel.app/`) — browser `Origin` headers never include a trailing slash, so the exact-match CORS check silently failed every request with a generic `500` (not a helpful CORS-specific error, since our `cors` middleware throws on mismatch and that error falls through to the global handler).
- Vercel's `REACT_APP_API_URL` was set to just the bare hostname (`b-spacetennisbooking-production.up.railway.app`), missing both `https://` and `/api`. Without a scheme, the browser treats it as a relative path and resolves it against Vercel's own origin instead of Railway's — request went to `https://vercel-app/b-spacetennisbooking-production.up.railway.app/auth/login` instead of the real backend.
- Railway's "Redeploy" button on an existing deployment entry does **not** necessarily pull the latest GitHub commit — it can rebuild the same stale commit. When in doubt, check the Deployments tab's list view for the actual commit message/hash on the Active entry, or check the file directly via the Console tab (`cat /app/<file> | grep <thing>`) rather than trusting deploy timestamps alone.
- `THAIBULKSMS_SENDER` on Railway had gotten set to an invalid custom brand string (`B-Space Tennis Club`, later `B-Space Tennis Space`) — both rejected by ThaiBulkSMS with `ERROR_SENDER`. Sender IDs must be ≤11 characters and, beyond `"Demo"`, require ThaiBulkSMS's approval process (see the still-open item below).

### Pending / In Progress
- [ ] **ThaiBulkSMS account is payment-gated for arbitrary recipients — blocks real customer OTP delivery.** ThaiBulkSMS's dashboard confirms the account is restricted to their "trial send" mode: custom message content can only be delivered to the account's own verified number (`0819118266`); any other recipient number can only receive ThaiBulkSMS's own fixed placeholder text, not our actual OTP content. Every number used for testing in this session (`0864123340`) was **not** that verified number, so those test sends likely also got the substituted placeholder rather than real OTP text (never explicitly confirmed either way). **This is the real explanation for the earlier "generic test message" mystery** — it was never about trial vs. corporate credit or the `force` parameter; those were real bugs worth fixing, but this recipient-restriction is the actual gate on going live. **Action needed: the account owner must pay ThaiBulkSMS (upgrade past trial) before any real customer can receive a working OTP via SMS.** Not a code issue — nothing to fix here, just an external dependency to close out before public launch. A full "real OTP to an arbitrary customer number" smoke test should be re-attempted once that payment/upgrade is done.
- [ ] **ThaiBulkSMS approved sender name** — still sends as `"Demo"`. Getting a real branded sender (e.g. `BSPACE`, ≤11 characters) requires submitting a request through ThaiBulkSMS's dashboard/support and waiting for approval (days, not minutes) — not a code change, just an env var swap once approved. Likely bundled with the same account upgrade as the item above.
...
4. ~~**Deploy backend to Railway**~~ — done, live at `https://b-spacetennisbooking-production.up.railway.app`. Root Directory set to `backend` (monorepo — Railway would otherwise try to build from repo root). Region: Singapore. All env vars set via the Variables tab's Raw Editor (`DATABASE_URL`, fresh production `JWT_SECRET`/`JWT_REFRESH_SECRET`, `NODE_ENV=production`, `THAIBULKSMS_API_KEY`/`SECRET`/`SENDER`, `SUPABASE_URL`/`SERVICE_ROLE_KEY`/`BUCKET`, `APP_NAME`, `OTP_EXPIRE_MINUTES`, `JWT_EXPIRE`, `JWT_REFRESH_EXPIRE`) — `FRONTEND_URL` deliberately deferred to step 6, `PORT` deliberately omitted (Railway injects its own). Verified for real: `GET /api/health` returns `200` with `db: "postgresql"`, protected routes correctly return `401` without a token, `POST /api/auth/login` correctly returns `404` for the unseeded production DB. **One gotcha hit**: first deploy crashed with `Environment variable not found: DATABASE_URL` — root cause was simply deploying before any env vars were set, resolved once the Variables block above was added.
5. ~~**Deploy frontend to Vercel**~~ — done, live at `https://b-space-tennis-booking.vercel.app`, Root Directory `frontend`. **Gotcha hit**: `REACT_APP_API_URL` was initially set to just the bare hostname, missing `https://` and `/api` — CRA/Vercel doesn't validate this, it just silently bakes whatever string is given into the build, and the browser treated the malformed value as a relative path against Vercel's own origin instead of Railway's. Fixed by setting the full URL and triggering a redeploy (Vercel bakes env vars in at build time, so changing the variable alone isn't enough).
6. ~~**Circle back to Railway**~~ — done, `FRONTEND_URL` set to the real Vercel URL. **Gotcha hit**: first attempt had a trailing slash, which fails the exact-match CORS check silently (browser `Origin` headers never have a trailing slash) — every request 500'd with no CORS-specific error message until that was removed.
7. ~~**Restrict Socket.IO CORS**~~ — done, see `Complete` list above.
8. **Smoke test on production URLs** — partially done: real SMS OTP login verified end-to-end (confirms step 3 and 5 both work together). Still to do: a full booking including payment slip upload (confirm the slip survives a Railway redeploy — confirms step 1 truly works, not just the upload endpoint), verify admin panel loads in Thai, verify Socket.IO slot updates work across two browser tabs.
